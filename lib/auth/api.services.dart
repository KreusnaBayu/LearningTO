...
class ApiService {
  ...
  static Future<void> signup(String name, String email, String password) async {
    ...
    try {
      ...
      if (response.data['status_code'] == 200) {
        ...
        print(
            'Signup failed with status code: ${response.data['status_code']}');
        print('Error messages: ${response.data['messages']}');
        throw Exception(
            'Signup failed with status code: ${response.data['status_code']}, Error messages: ${response.data['messages']}');
      }
    } catch (error) {
  ...
  static Future<Question> getQuestionById(int id) async {
    final apiUrl =
        '$baseUrl/questions/$id'; // Ganti dengan API endpoint yang sesuai
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print(
            'Failed to load question with status code: ${response.statusCode}');
        throw Exception('Failed to load question');
      }
    } catch (error) {
      print('Error during fetching question: $error');
      throw Exception('Error during fetching question: $error');
    }
  }
}
  ...
  static Future<Map<String, dynamic>> validateCredentials(
    ...
    try {
      ...
      if (response.data['status_code'] == 200) {
        ...
        authToken =
            response.data['data']['access_token']; // Simpan token setelah login
        ...
        print(
            'Validation failed with status code: ${response.data['status_code']}');
    ...
    try {
      ...
      if (userData.isNotEmpty) {
        ...
        if (response.data['status_code'] == 200) {
          ...
          print(
              'Login failed with status code: ${response.data['status_code']}');
          print('Response body: ${response.data}');
        }
      } else {
